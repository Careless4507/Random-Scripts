...
isoinfo -i "VBoxGuestAdditions_$LATEST_VERSION.iso" -x "/VBoxLinuxAdditions.run" > "$HOME/vbox/VBoxLinuxAdditions.run"
if [ $? -ne 0 ]; then
  echo -e '\e[91mFailed to extract ISO contents\e[0m'
  exit 1
else
  echo -e '\e[96mISO contents extracted!\e[0m'
  chmod +x "$HOME/vbox/VBoxLinuxAdditions.run"
  echo -e '\e[96mFile made executable! Run the install script located at '$HOME/vbox/VBoxLinuxAdditions.run'\e[0m'
